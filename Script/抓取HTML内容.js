const $ = new Env("抓取HTML内容");

const args = Object.fromEntries($argument.split("&").map((item) => item.split("=", 2)));

(async () => {
    $.CheerioJS = await loadCheerioJS();
    const Cheerio = $.CheerioJS.load(await loadHTML());
    console.log(`✅ ${$.name}: 开始抓取 ${args.path}`);

    const content = Cheerio(args.path)
        .map((_, el) => Cheerio(el).text())
        .get()
        .join("\n");

    console.log(content);
    console.log(`✅ ${$.name}: 成功获取到数据`);
    $.done({
        title: args.name,
        content: content,
        icon: args.icon || "h.square",
...
async function loadHTML() {
    return new Promise(async (resolve) => {
        $.getScript(decodeURIComponent(args.url)).then((data) => {
            console.log(`✅ ${$.name}: 加载 HTML 成功`);
            resolve(data);
        });
    });
}
...
// 加载CheerioJS模块
async function loadCheerioJS() {
    let code = $.getdata("Cheerio_code") || "";
    //ios环境
    if (code && Object.keys(code).length) {
        console.log(`✅ ${$.name}: 缓存中存在 CheerioJS 代码, 跳过下载`);
        eval(code);
        return createCheerio();
    }
    console.log(`🚀 ${$.name}: 开始下载 CheerioJS 代码`);
    return new Promise(async (resolve) => {
        $.getScript("https://raw.githubusercontent.com/Sliverkiss/QuantumultX/main/Utils/cheerio.js").then((fn) => {
            $.setdata(fn, "Cheerio_code");
            eval(fn);
            console.log(`✅ CheerioJS 加载成功`);
            const CheerioJS = createCheerio();
            resolve(CheerioJS);
        });
    });
}
    ...
    return new (class {
        ...
        runScript(t, e) {
            return new Promise((s) => {
                ...
                const [r, a] = i.split("@"),
                    n = { url: `http://${a}/v1/scripting/evaluate`, body: { script_text: t, mock_type: "cron", timeout: o }, headers: { "X-Key": r, Accept: "*/*" }, policy: "DIRECT", timeout: o };
