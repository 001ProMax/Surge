...
let history = JSON.parse($persistentStore.read("RULEMATCHINGTIME") || JSON.stringify(initial));
...
$httpAPI("GET", "/v1/requests/recent", null, (body) => {
    ...
    let avgMatchingTime = ((history.totalMatchingTime * 1000) / history.numberOfRequests).toFixed($argument);
    $done({
